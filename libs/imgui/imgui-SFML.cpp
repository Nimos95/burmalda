...
bool Init(sf::Window& window, const sf::Vector2f& displaySize, bool loadDefaultFont) {
    ...
    // ✅ ДОБАВЛЯЕМ ПОДДЕРЖКУ РУССКОГО ШРИФТА
    if (loadDefaultFont) {
        // Пытаемся загрузить шрифт с поддержкой кириллицы
        ImFont* font = io.Fonts->AddFontFromFileTTF("config/arial.ttf", 24.0f, nullptr, io.Fonts->GetGlyphRangesCyrillic());

        if (!font) {
            // Если не удалось загрузить arial.ttf, пробуем стандартный шрифт
            std::cout << "Failed to load arial.ttf, using default font" << std::endl;
            io.Fonts->AddFontDefault();
        }
        else {
            std::cout << "Loaded Russian font: config/arial.ttf" << std::endl;
        }
